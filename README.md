...
- Error message says you must run the sequential benchmark first  
  Go to `sequential` and run:
...
- Plot files are empty or missing  
  Delete old plots and regenerate:

  ```bash
  cd mergesort
  rm -f plots/*.png plots/*.pdf
  make plot
  python3 plots.py        # if you want submission_mergesort.pdf
  ```
...
- A sequential and a parallel implementation of mergesort.
- A benchmarking pipeline controlled by `params.sh`.
- Plots in `mergesort/plots/` that show speedup, time, and efficiency.
- Experimental evidence that:
  - For small problem sizes, parallelization overhead dominates.
  - For large problem sizes, you can get 2 to 4 times speedup compared to the sequential baseline.
  - Overall scaling is limited by memory bandwidth and serial or overhead parts of the algorithm.

You can directly include the generated plots and `submission_mergesort.pdf` in reports or homework submissions.
